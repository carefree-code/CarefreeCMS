...
### 4. API 路由 ✅
```
GET  /api/templates/themes          # 扫描所有模板套装
GET  /api/templates/current-theme   # 获取当前模板套装
POST /api/templates/switch-theme    # 切换模板套装
GET  /api/templates/scan            # 扫描模板文件
```
...
```bash
# 复制 default 主题为基础
cp -r api/templates/default api/templates/mytheme

# 修改配置文件
nano api/templates/mytheme/theme.json
...
1. **备份现有模板**
   ```bash
   cp -r api/templates api/templates_backup
   ```
...
<html>
<head>
    <title>{$title|default=$config.web_name}</title>
...
- `api/app/controller/api/Template.php` - 模板管理控制器
- `api/app/controller/api/Build.php` - 静态生成控制器
- `backend/src/api/template.js` - 前端API调用
- `backend/src/views/config/Index.vue` - 系统配置界面
